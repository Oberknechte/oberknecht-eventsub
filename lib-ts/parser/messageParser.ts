...
export function messageParser(
  ...
  const responseParsed =
    messageTypes[messageType] && isConstructor(messageTypes[messageType])
      ? new messageTypes[messageType](sym, wsNum, response)
      : undefined;
  ...
  switch (messageType) {
    case "session_welcome": {
      ...
      i.OberknechtEmitter[sym].emit(
        [`ws:${wsNum}:session_welcome`, "session_welcome", "sessionwelcome"],
        responseParsed ?? response
      );
